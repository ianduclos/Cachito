...
import { countsTowardBid, isHigherBid, MAX_PLAYERS, type Bid, type Die, type GameAction, type GameRules, type LegalActions, type PublicGameView, type PublicPlayer } from "../engine";
...
import { GameSummary } from "./OnlineGameAnalysis";
...
function OnlineGameContent({ onExit, onReconnect, restoreSavedSession = false }: { onExit: () => void; onReconnect: () => void; restoreSavedSession?: boolean }) {
  ...
  const [analysis, setAnalysis] = useState<MatchAnalysis | undefined>();
  ...
  // Leaving the winner screen is each player's own call: whoever asks first takes
  // the room back to its lobby, but everybody else keeps reading the summary until
  // they press it too. This flag is that press.
    ...
    const connect = () => {
      ...
      connection.onmessage = (event) => {
        ...
        if (message.type === "joined") {
          ...
          setConnected(true); setRoomCode(message.roomCode); setPlayerId(message.playerId); setError(null);
        ...
        if (message.type === "lobby") { setLobby(message); setView((current) => current?.phase === "gameOver" ? current : null); setLegal(undefined); setAnnouncement(undefined); setShuffle(undefined); setNextRound(undefined); setPaused(undefined); setTurnDeadlineAt(undefined); }
        if (message.type === "state") { setView(message.view); setLeftSummary(false); setAnalysis(message.analysis); setLobby(null); setLegal(message.legalActions); setHistory(message.history); setAnnouncement(message.announcement); setShuffle(message.shuffle); setNextRound(message.nextRound); setPaused(message.paused); setPlayerStatuses(message.playerStatuses); setTurnDeadlineAt(message.turnDeadlineAt); }
      };
    };
    connect();
  ...
  const holdingSummary = view?.phase === "gameOver" && !leftSummary;
  ...
  if (lobby && !holdingSummary) return <><LobbyScreen lobby={lobby} playerId={playerId} connected={connected} error={error} onStart={() => send(socket.current, { type: "start-game" })} onAddBot={() => send(socket.current, { type: "add-bot" })} onRemoveBot={(botId) => send(socket.current, { type: "remove-bot", playerId: botId })} onKickPlayer={(targetId) => send(socket.current, { type: "kick-player", playerId: targetId })} onRename={(nextName) => { localStorage.setItem("cachito-display-name", nextName); send(socket.current, { type: "rename-player", name: nextName }); }} onProposeRules={(rules) => send(socket.current, { type: "propose-rules", rules })} onApproveRules={() => send(socket.current, { type: "approve-rules" })} onExit={leaveRoom} /><div className="menu-settings"><GameSettings /></div></>;
  return <OnlineTable view={view!} analysis={analysis} roomCode={roomCode} history={history} legal={legal} playerId={playerId} playerStatuses={playerStatuses} turnDeadlineAt={turnDeadlineAt} connected={connected} error={error} announcement={announcement} shuffle={shuffle} nextRound={nextRound} paused={paused} onExit={leaveRoom} onPause={() => send(socket.current, { type: "toggle-pause" })} onForfeit={() => send(socket.current, { type: "forfeit-game" })} onShuffle={() => send(socket.current, { type: "shuffle-dice" })} onReadyNextRound={() => send(socket.current, { type: "ready-next-round" })} onAction={(action) => send(socket.current, { type: "action", action })} lobbyWaiting={Boolean(lobby)} onReturnToLobby={() => { if (!lobby) send(socket.current, { type: "return-to-lobby" }); setLeftSummary(true); }} />;
}
...
function OnlineTable({ view, analysis, roomCode, history, legal, playerId, playerStatuses, turnDeadlineAt, connected, error, announcement, shuffle, nextRound, paused, lobbyWaiting, onPause, onForfeit, onShuffle, onReadyNextRound, onAction, onReturnToLobby, onExit }: { lobbyWaiting: boolean; view: PublicGameView; analysis?: MatchAnalysis; roomCode: string; history: string[]; legal?: LegalActions; playerId?: string; playerStatuses: Array<{ id: string; connected: boolean; covered: boolean }>; turnDeadlineAt?: number; connected: boolean; error: string | null; announcement?: { text: string; playerId?: string }; shuffle?: Shuffle; nextRound?: NextRound; paused?: Pause; onPause: () => void; onForfeit: () => void; onShuffle: () => void; onReadyNextRound: () => void; onAction: (action: GameAction) => void; onReturnToLobby: () => void; onExit: () => void }) {
  ...
  // Table dice must back the claim, so only dice counting toward the denomination
  // being bid can be selected — wild aces included outside Palo Fijo and ace bids.
  const tableDieEligible = useCallback((value: number) => countsTowardBid(value as Die, { quantity, denomination: selectedDenomination }, view.paloFijo), [quantity, selectedDenomination, view.paloFijo]);
  const eligibleTableDice = ownHand?.filter((die) => tableDieEligible(die)).length ?? 0;
  // Derived rather than stored: changing the denomination can invalidate a die
  // picked under the previous one, so the live selection is the eligible subset.
  const tableDiceSelection = ownHand ? tableDiceIndices.filter((index) => tableDieEligible(ownHand[index])) : [];
  const canPutDiceOnTable = Boolean(legal?.canPutDiceOnTable && ownHand && eligibleTableDice);
  ...
  // your immediate left, and turn order sweeps up the left side and down the right
  // to the player right before you on your right. view.players is in turn order,
  // so rotate it to start just after your own seat before filtering yourself out.
  ...
  const bid = () => {
    if (!connected) return;
    stopClockSound();
    const tableReroll = tableDiceMode && tableDiceSelection.length > 0;
    ...
    onAction({ type: "bid", playerId: "", bid: { quantity, denomination: selectedDenomination }, ...(tableDiceMode && tableDiceSelection.length ? { tableDiceIndices: tableDiceSelection } : {}) });
  ...
  useEffect(() => {
    ...
    // Opening a round always starts from the lowest claim available. Keeping the
    // previous round's quantity — still legal, since an opening bid has nothing
    // to beat — made it far too easy to open on 9 by reflex.
    if (view.currentBid && availableBids.some((bid) => bid.quantity === quantity && bid.denomination === selectedDenomination)) {
  ...
  return <main className={`table-prototype-shell online-live-table${view.phase === "gameOver" ? " online-live-table--game-over" : ""}${reducedMotion ? " game-shell--reduced-motion" : ""}`}>
    <header className="tp-header online-table-header"><div><p className="tp-eyebrow">Online table · Room {roomCode}</p><h1>Cachito</h1></div><div className="tp-header-actions"><span className={`online-connection-pill${connected ? "" : " online-connection-pill--offline"}`}>{connected ? "Live" : "Reconnecting"}</span><span className="online-round-pill">Round {view.round}{view.paloFijo ? " · Palo fijo" : ""}</span><button className="tp-quiet-button tp-feed-toggle" type="button" aria-expanded={feedOpen} onClick={() => setFeedOpen((open) => !open)}>Activity <span>{history.length}</span></button><div className="online-settings-anchor"><button className="settings-button" ref={settingsButtonRef} aria-expanded={settingsOpen} aria-controls="online-game-settings" aria-label="Game settings" onClick={() => { setSettingsOpen((open) => !open); setConfirmingForfeit(false); }}>⚙</button>{settingsOpen && <div className="settings-popover" id="online-game-settings" ref={settingsPopoverRef} role="dialog" aria-label="Game settings" tabIndex={-1}><strong>Settings</strong><label><input type="checkbox" checked={reducedMotion} onChange={(event) => setReducedMotion(event.target.checked)} /> Reduce motion</label><label className="sound-slider">Sound FX <input type="range" min="0" max="1" step="0.05" value={soundLevels.effects} onChange={(event) => changeSoundLevel("effects", Number(event.target.value))} /><output>{Math.round(soundLevels.effects * 100)}%</output></label><label className="sound-slider">Music <input type="range" min="0" max="1" step="0.05" value={soundLevels.music} onChange={(event) => changeSoundLevel("music", Number(event.target.value))} /><output>{Math.round(soundLevels.music * 100)}%</output></label>{playerId && !eliminated && view.phase !== "gameOver" && <>{!confirmingForfeit ? <button className="button button--ghost settings-forfeit" disabled={!connected} onClick={() => setConfirmingForfeit(true)}>Forfeit game</button> : <div className="settings-forfeit-confirm"><small>You’ll be out and continue as a spectator.</small><button className="button settings-forfeit-danger" disabled={!connected} onClick={() => { onForfeit(); setConfirmingForfeit(false); setSettingsOpen(false); }}>Confirm forfeit</button><button className="button button--ghost" onClick={() => setConfirmingForfeit(false)}>Keep playing</button></div>}</>}{playerId && view.phase !== "gameOver" && <button className="button button--ghost settings-pause" disabled={!connected} onClick={onPause}>{paused ? "Resume game" : "Pause game"}</button>}<button className="button button--ghost settings-exit" disabled={!connected} onClick={onExit}>Exit to menu</button></div>}</div></div></header>
    ...
    <div className={`tp-play-layout${feedOpen ? " tp-play-layout--feed-open" : " tp-play-layout--feed-closed"}`}>
      <section className="tp-stage" aria-label={`${view.players.length}-player online table`}>
        <div className={`tp-table${shufflingDice ? " tp-table--shuffling" : ""}`}>
          ...
          {view.paloFijo && <div className="online-palo-banner"><strong>Palo fijo</strong><span>Aces are not wild{ownPlayer?.diceCount === 1 ? " · your die may switch denomination, aces highest" : view.rules.paloFijoBlindDice && (ownPlayer?.diceCount ?? 0) > 1 ? " · your hand stays hidden" : ""}</span></div>}
