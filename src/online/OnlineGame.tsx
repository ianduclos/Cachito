...
function OnlineGameContent({ onExit, onReconnect, restoreSavedSession = false }: { onExit: () => void; onReconnect: () => void; restoreSavedSession?: boolean }) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      connection.onmessage = (event) => {
        ...
        if (message.type === "lobby") { setLobby(message); setHostPlayerId(message.hostPlayerId); setView(null); setAnalysis(undefined); setLegal(undefined); setAnnouncement(undefined); setShuffle(undefined); setNextRound(undefined); setPaused(undefined); setPlayerStatuses([]); setTurnDeadlineAt(undefined); }
        if (message.type === "state") { setHostPlayerId(message.hostPlayerId); setView(message.view); setAnalysis(message.analysis); setLobby(null); setLegal(message.legalActions); setHistory(message.history); setAnnouncement(message.announcement); setShuffle(message.shuffle); setNextRound(message.nextRound); setPaused(message.paused); setPlayerStatuses(message.playerStatuses); setTurnDeadlineAt(message.turnDeadlineAt); }
      };
    };
    connect();
  ...
  return <OnlineTable view={view!} analysis={analysis} roomCode={roomCode} history={history} legal={legal} playerId={playerId} playerStatuses={playerStatuses} turnDeadlineAt={turnDeadlineAt} connected={connected} error={error} isHost={playerId === hostPlayerId} announcement={announcement} shuffle={shuffle} nextRound={nextRound} paused={paused} onExit={leaveRoom} onPause={() => send(socket.current, { type: "toggle-pause" })} onForfeit={() => send(socket.current, { type: "forfeit-game" })} onShuffle={() => send(socket.current, { type: "shuffle-dice" })} onReadyNextRound={() => send(socket.current, { type: "ready-next-round" })} onAction={(action) => send(socket.current, { type: "action", action })} onReturnToLobby={() => send(socket.current, { type: "return-to-lobby" })} />;
}
    ...
    <div className={`tp-play-layout${feedOpen ? " tp-play-layout--feed-open" : " tp-play-layout--feed-closed"}`}>
      <section className="tp-stage" aria-label={`${view.players.length}-player online table`}>
        <div className={`tp-table${shufflingDice ? " tp-table--shuffling" : ""}`}>
          ...
          {view.phase === "gameOver" && <GameSummary view={view} analysis={analysis} history={history} connected={connected} isHost={isHost} onReturnToLobby={onReturnToLobby} onExit={onExit} />}
