...
function OnlineGameContent({ onExit, onReconnect, restoreSavedSession = false }: { onExit: () => void; onReconnect: () => void; restoreSavedSession?: boolean }) {
  ...
  return <OnlineTable view={view!} roomCode={roomCode} history={history} legal={legal} playerId={playerId} playerStatuses={playerStatuses} turnDeadlineAt={turnDeadlineAt} connected={connected} error={error} isHost={playerId === hostPlayerId} announcement={announcement} shuffle={shuffle} nextRound={nextRound} paused={paused} onExit={leaveRoom} onPause={() => send(socket.current, { type: "toggle-pause" })} onForfeit={() => send(socket.current, { type: "forfeit-game" })} onShuffle={() => send(socket.current, { type: "shuffle-dice" })} onReadyNextRound={() => send(socket.current, { type: "ready-next-round" })} onAction={(action) => send(socket.current, { type: "action", action })} onReturnToLobby={() => send(socket.current, { type: "return-to-lobby" })} />;
}
