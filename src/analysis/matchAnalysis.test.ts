...
const botDecision: BotDecisionRecord = {
  ...
  trace: { model: 'persona-bluff', version: 1, decisionReason: 'controlled_bluff', candidateCount: 20, consideredCandidates: [], random: {}, settings: {}, plainReason: 'It found a cheap moment to sell a believable story on a face it genuinely held.' },
  probabilities: {},
}
...
describe('completed match analysis', () => {
  it('turns verified outcomes and privacy-safe bot traces into readable player summaries', () => {
    ...
    expect(analysis.players.find((player) => player.id === 'bot')).toMatchObject({
      ...
      stats: { confirmedBluffs: 1, bluffsCaught: 1 },
