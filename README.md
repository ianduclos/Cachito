...
5. Once all active cups are ready, every turn shows the room's configured timer (one minute by default). While waiting, a seated player can prepare quantity and denomination; only the active player can submit, call Dudo/Calzo, or place table dice. Bots act after a 3–8 second thinking pause, not a shortened visible timer.
...
The final-ten-seconds clock cue stops immediately when a move resolves. Sound clips are preloaded in reusable two-voice pools so network fetch and decode do not delay timing-sensitive cues; the intentional Dudo/Calzo suspense pauses remain unchanged. Theme music ducks for longer effects, and the settings menu controls music and effects separately. A reconnect token stored in the browser restores an existing player after a connection loss when the room is still active; the Online menu includes a subtle **Reconnect to saved game** action when one is available. While reconnecting, the current lobby or table remains visible with a status banner, but all server-changing controls are disabled so moves are not silently discarded.
...
- **Turn clocks are monotonic within a turn.** Do not publish a new actor with the previous actor's deadline or restart an unchanged actor's timer for unrelated room events. Pausing preserves the exact remaining duration; offline bot cover may shorten a clock but must never add time.
- **Forfeit is explicit and confirmed.** An active player can forfeit from Settings, which eliminates that seat, keeps it visible as a spectator, clears the interrupted bid, and begins a clean round for the remaining players (or immediately declares the last player the winner).
- **Online room residency is deliberate:** the Cloud Run service has a service-level maximum of one instance and concurrency 80 because current room state is in-memory. The supported beta target is four simultaneous full eight-player games on that authoritative process, leaving roughly 48 of the 80 connection slots for spectators and reconnect overlap. This is a validated capacity target, not a four-instance setting: raising the instance count would split the in-memory room map and break joins until room coordination moves to shared durable storage. Deployments disconnect live sockets, which clients then recover through reconnect tokens and snapshots. Active-room snapshots are private, throttled/coalesced recovery records.
...
Logs are intended for local inspection now and later batch analysis. Every new local game log, online active-room recovery record, and private online match snapshot includes `gameVersion` using the visible `rYYYY.MM.DD.NNN` release marker as well as its independent `schemaVersion`. Use `gameVersion` to identify the game build and `schemaVersion` to select a parser; group comparisons by seed and seat assignment, and treat decision probabilities as predictions to score against later public round resolutions. Logs can become evaluation or self-play examples without granting a learner information that the acting bot did not have at decision time.
